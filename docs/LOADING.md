# NGA 镜像站 — 加载性能优化策略 v1.0

> 审计日期: 2026-05-22 | 覆盖范围: 全栈 (网络 / JS / CSS / 渲染 / I/O)
...
| 论坛页 | ~800ms* | 105 kB | 87 kB | SSR → **客户端重复 fetch*** |
| 帖子页 | ~1.2s* | 106 kB | 87 kB | SSR → **客户端重复 fetch*** |

> *论坛页和帖子页目前 SSR 数据被浪费——客户端总是重新 fetch。
...
**问题**: `BottomNav.tsx:58-63` 使用原生 `<a href>` 而非 Next.js `<Link>`，每次底部导航触发**完整页面重新加载**，下载全部 JS/CSS/HTML，重跑所有初始化逻辑。

**影响**: 移动端添加 1-3s 延迟（3G 网络）。所有手机用户每次切换 tab 都经历。

**修复**:
```diff
- <a key={tab.href} href={tab.href} onClick={(e) => handleTap(tab.href, e)}>
+ <Link key={tab.href} href={tab.href} onClick={(e) => handleTap(tab.href, e)}>
```
...
**问题**: `ForumPageClient.tsx` 和 `ThreadPageClient.tsx` 的 SSR 注入流程有缺陷：

1. 服务端 fetch 数据 → 以 props 传入
2. Client useEffect#1 将数据写入 Zustand `forum-store`
3. Client useEffect#2 检查 `cache-store`(始终为空) → **发起重复 fetch**
4. 重复 fetch 覆盖 store.loading = true → **SSR 内容被 spinner 替换** → 网络回来后恢复

**影响**: 每次 SSR 页面 = 双重加载。浪费 200-800ms + 额外服务端请求。

**修复**:
1. SSR useEffect 同时写入 `cacheStore.set(key, data)`
2. useEffect#2 先检查 `store.threads.length > 0` 再决定是否 fetch

**效率**: 论坛/帖子页首屏 -50% 延迟 (消除 spinner flash + 网络往返)

**文件**: `ForumPageClient.tsx`, `ThreadPageClient.tsx`

---

#### A3: `console.log` 同步 I/O

**问题**: `logger.ts:22-25` 对**每次 API 请求**执行同步 `console.log()`。Node.js 中 `console.log` 是阻塞 I/O (stdout write + pipe flush)。

**影响**: 100 req/s → 消耗 5-15% 事件循环。生产环境无需。
